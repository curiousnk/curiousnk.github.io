...
/** Clear Adobe ECID cookies (first-party). Adobe sets AMCV_*, AMCVS_* on current domain. */
function clearAdobeECIDCookies() {
    var cookies = document.cookie.split(';');
    for (var i = 0; i < cookies.length; i++) {
        var name = cookies[i].split('=')[0].replace(/^\s+|\s+$/g, '');
        if (name.indexOf('AMCV') === 0 || name.indexOf('AMCVS') === 0 || name === 'demdex' || name.indexOf('s_ecid') === 0) {
...
function getProductImage(productName) {
    ...
    const productImages = {
        ...
        'Desk Lamp': 'https://images.unsplash.com/photo-1556909114-f6e7ad7d3136?w=400&q=80'
    };
